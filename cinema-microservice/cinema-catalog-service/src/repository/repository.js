...
function getAllCities(cb) {
    mongodb.connect((err, db) => {
        db.collection('cinemaCatalog').find({}, {cidade:1, uf:1, pais:1}).toArray(cb)
    })
}

function getCinemasByCityId(cityId, cb) {
    ...
    mongodb.connect((err, db) => {
        db.collection('cinemaCatalog').find({_id: objCityId}, {cinemas: 1}).toArray((err, cities) => {
            ...
            cb(err, cities[0].cinemas)
        })
    })
}

function getMoviesByCinemaId(cinemaId, cb) {
...
function getMoviesByCityId(cityId, cb) {
    var objCityId = ObjectId(cityId)
    mongodb.connect((err, db) => {
        db.collection('cinemaCatalog').aggregate([
            {$match: {'_id': objCityId}},
            ...
            cb(err, sessions.map(item => { return {idFilme: item._id.idFilme, filme: item._id.filme }}))
        })
    })
}

function getMovieSessionsByCityId(movieId, cityId, cb) {
    ...
    mongodb.connect((err, db) => {
        db.collection("cinemaCatalog").aggregate([
            ...
            if(err) return cb(err, null);
            cb(err, sessions.map(item => { return {idFilme: item._id.idFilme, filme: item._id.filme, idCinema: item._id.idCinema, sala: item._id.sala, sessao: item._id.sessao } } ))
        })
    })
}

function getMovieSessionsByCinemaId(movieId, cinemaId, cb){
    var objCinemaId = ObjectId(cinemaId);
    var objMovieId = ObjectId(movieId);
    mongodb.connect((err, db) => {
        db.collection("cinemaCatalog").aggregate([
            ...
            if(err) return cb(err, null);
            cb(err, sessions.map(item => { return {idFilme: item._id.idFilme, filme: item._id.filme, sala: item._id.sala, sessao: item._id.sessao } } ))
        })
    })
}
